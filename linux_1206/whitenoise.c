...
int main(int argc, char** argv) {
	...
	/*for(i=0; i<height*3; i+=3) { 
	 ...
	 for(i=0; i<height*3; i+=3) {
		for(j=0; j<width*3; j+=3) {
			outimg[j+width*i+0] = inimg[j+(i*width+0)];
			outimg[j+width*i+1] = inimg[j+(i*width+1)];
			outimg[j+width*i+2] = inimg[j+(i*width+2)];
		}
	 }
	...
	for(i=0; i<atoi(argv[3]); i++) {
		...
		int g = rand()&0xFF;
		int b = rand()&0xFF;*/
		int whitenoise = rand()&0xFF;

		/*outimg[randwidth*3+width*randheight*3+0] = clip(r, 0, 255);
		outimg[randwidth*3+width*randheight*3+1] = clip(g, 0, 255);
		outimg[randwidth*3+width*randheight*3+2] = clip(b, 0, 255);*/
		outimg[randwidth*3+width*randheight*3+0] = clip(whitenoise, 0, 255);
		outimg[randwidth*3+width*randheight*3+1] = clip(whitenoise, 0, 255);
		outimg[randwidth*3+width*randheight*3+2] = clip(whitenoise, 0, 255);
	}
