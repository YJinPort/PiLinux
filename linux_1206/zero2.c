...
int main(int argc, char** argv) 
{
    ...
    int padSize = (bmpInfoHeader.biWidth + 4) * elemSize;
    int addSize = (padSize + bmpInfoHeader.biHeight)*4;
    ...
    for(y = 0; y < bmpInfoHeader.biHeight; y++) {
        for(x = 0; x < bmpInfoHeader.biWidth * elemSize; x+=elemSize) {
            for(z = 0; z < elemSize; z++) {
                padimg[(x+elemSize*2)+(y+2)*padSize+z]=inimg[x+y*size+z];
            }
        }
    }
    ...
    for(y = 0; y < bmpInfoHeader.biHeight; y++) { 
        for(z = 0; z < elemSize; z++) {
            padimg[0+(y+2)*padSize+z]=inimg[0+y*size+z];
            padimg[padSize-elemSize+(y+2)*padSize+z]=inimg[size-elemSize+y*size+z];
        }
    }

    for(x = 0; x < bmpInfoHeader.biWidth*elemSize; x++) { 
        padimg[elemSize*2+x]=inimg[x];
        padimg[elemSize*2+x+(bmpInfoHeader.biHeight+2)*padSize]=inimg[x+(bmpInfoHeader.biHeight-1)*size];
    }
            ...
            for(z = 0; z < elemSize; z++) {
                ...
                for(int i = -1; i < 4; i++) {
                    for(int j = -1; j < 4; j++) {
						...
						pixel[cnt++] = padimg[(x+i*elemSize)+(y+j)*padSize+z];
                    }
                }
				insertion(pixel, 25);
                //outimg[(x-elemSize)+(y-1)*size+z] = LIMIT_UBYTE(sum);
                outimg[(x-elemSize)+(y-1)*size+z] = pixel[12];
				cnt = 0;
            }
        }
    }         
