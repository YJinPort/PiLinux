...
int main(int argc, char** argv) 
{
	...
	for(y = 0; y < bmpInfoHeader.biHeight; y++) {
		for(x = 0; x < size; x+=elemSize) {
			...
			grayimg[x+y*size+0] = ((66*r+129*g+25*b+128)>>8)+16;
			grayimg[x+y*size+1] = ((66*r+129*g+25*b+128)>>8)+16;
    ...
    for(y = 1; y < bmpInfoHeader.biHeight + 1; y++) { 
        for(x = elemSize; x < padSize; x+=elemSize) {
            for(z = 0; z < elemSize; z++) {
                ...
                for(int i = -1; i < 2; i++) {
                    for(int j = -1; j < 2; j++) {
                        xvalue += xkernel[i+1][j+1]*padimg[(x+i*elemSize)+(y+j)*padSize+z];
                        yvalue += ykernel[i+1][j+1]*padimg[(x+i*elemSize)+(y+j)*padSize+z];
                    }
                }
                outimg[(x-elemSize)+(y-1)*size+z] = LIMIT_UBYTE(sqrt(xvalue*xvalue+yvalue*yvalue));
            }
        }
    }         
